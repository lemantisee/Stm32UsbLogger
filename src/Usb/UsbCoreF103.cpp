...
PCD_HandleTypeDef &UsbCoreF103::getPcdHandle() { return mPcd; }

bool UsbCoreF103::openEndpoint(UsbHandle *pdev, uint8_t ep_addr, UsbHandle::EndpointType enType,
                               uint16_t ep_mps)
{
    return HAL_PCD_EP_Open((PCD_HandleTypeDef *)pdev->pData, ep_addr, ep_mps, uint8_t(enType))
           == HAL_OK;
}

bool UsbCoreF103::closeEndpoint(UsbHandle *pdev, uint8_t ep_addr)
{
    return HAL_PCD_EP_Close((PCD_HandleTypeDef *)pdev->pData, ep_addr) == HAL_OK;
}

bool UsbCoreF103::flushEndpoint(UsbHandle *pdev, uint8_t ep_addr)
{
    return HAL_PCD_EP_Flush((PCD_HandleTypeDef *)pdev->pData, ep_addr) == HAL_OK;
}

bool UsbCoreF103::stallEndpoint(UsbHandle *pdev, uint8_t ep_addr)
{
    return HAL_PCD_EP_SetStall((PCD_HandleTypeDef *)pdev->pData, ep_addr) == HAL_OK;
}

bool UsbCoreF103::clearStallEndpoint(UsbHandle *pdev, uint8_t ep_addr)
{
    return HAL_PCD_EP_ClrStall((PCD_HandleTypeDef *)pdev->pData, ep_addr) == HAL_OK;
}

bool UsbCoreF103::isEndpointStall(UsbHandle *pdev, uint8_t ep_addr) const
{
...
bool UsbCoreF103::setUsbAddress(UsbHandle *pdev, uint8_t dev_addr)
{
    return HAL_PCD_SetAddress((PCD_HandleTypeDef *)pdev->pData, dev_addr) == HAL_OK;
}

bool UsbCoreF103::transmit(UsbHandle *pdev, uint8_t ep_addr, uint8_t *pbuf, uint16_t size)
{
    return HAL_PCD_EP_Transmit((PCD_HandleTypeDef *)pdev->pData, ep_addr, pbuf, size) == HAL_OK;
}

bool UsbCoreF103::prepareReceive(UsbHandle *pdev, uint8_t ep_addr, uint8_t *pbuf, uint16_t size)
{
    return HAL_PCD_EP_Receive((PCD_HandleTypeDef *)pdev->pData, ep_addr, pbuf, size) == HAL_OK;
}

uint32_t UsbCoreF103::getRxDataSize(UsbHandle *pdev, uint8_t ep_addr)
{
...
bool UsbCoreF103::initInterface(UsbHandle *pdev)
{
    /* Init USB Ip. */
...
bool UsbCoreF103::deinitInterface(UsbHandle *pdev)
{
    return HAL_PCD_DeInit((PCD_HandleTypeDef *)pdev->pData) == HAL_OK;
}

bool UsbCoreF103::startInterface(UsbHandle *pdev)
{
    return HAL_PCD_Start((PCD_HandleTypeDef *)pdev->pData) == HAL_OK;
}

bool UsbCoreF103::stopInterface(UsbHandle *pdev)
{
